        ...
        if ($products_id_master != $products_id_slave) {
			...
			if ($Qcheck->fetch() === false) {
			  ...
			  $Qinsert->execute();
				 ...
				 $OSCOM_MessageStack->add($OSCOM_Related->getDef('alert_product_added_success'), 'success', 'Related');
			} else {
				$OSCOM_MessageStack->add($OSCOM_Related->getDef('alert_product_added_failure'), 'warning', 'Related');
			}
		}

        $OSCOM_Related->redirect('Admin&products_id_master=' . $products_id_master);
    }
}